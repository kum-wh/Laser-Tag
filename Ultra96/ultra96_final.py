...
class AI():
    ...
    def ai(self):
        ...
        input_buffer = allocate(shape=(2124,), dtype=np.intc)
        ...
        # Change to the correct mapping later
    ...
    def run(self):
        while True:
            ...
            print(f"AI HAS RECEIVED SENSOR READING {sensor_reading}")

            # TODO gun shoot and miss
            if '1g' in sensor_reading_in_dict.keys() and sensor_reading_in_dict['1g'] == 1:
                action = 'shoot'
            ...
            if '1i' in sensor_reading_in_dict.keys():
                # P1, IMU
                # Must make sure have 6 arrays, each have a length of 300
                self.x.append(sensor_reading_in_dict['1i']['x']) 
                self.y.append(sensor_reading_in_dict['1i']['y']) 
                self.z.append(sensor_reading_in_dict['1i']['z']) 
                self.gx.append(sensor_reading_in_dict['1i']['gx']) 
                self.gy.append(sensor_reading_in_dict['1i']['gy']) 
                self.gz.append(sensor_reading_in_dict['1i']['gz']) 

                if (len(self.x) == self.MAXIMUM_COUNTER):
                    action = self.ai()
                    ...
                    if action == 'grenade':
                        queue_visualizer.put(action)
                        start = time.time()

                        # TODO
                        # possible time out required 

                        greande_hit_or_miss = queue_greande_hit_or_miss.get()
                        end = time.time()
                        print(f"VIZ RESPONDS IN {end - start} second(s)")
                        queue_game_state.put(greande_hit_or_miss)
                    else:
                        queue_game_state.put(action)
...
class GameMechanics():
    ...
    def run(self):
        while True:
            non_greande_action_or_greande_hit_or_miss = queue_game_state.get()
            if type(non_greande_action_or_greande_hit_or_miss) == str: 
                self.p1_action = non_greande_action_or_greande_hit_or_miss
            else:
                self.p1_action = 'grenade'
                if non_greande_action_or_greande_hit_or_miss == False: # GOT PROBLEMS? CAN EXCEED 100 WOR..
                    self.game_state.player_2.hp += 30 # IF GOT SET MAX IS 100 then die alr, can I just - then +?
                    # simple fix: set a flag to be true here and later on check whether it is true if true then add 30 back
            ...
            # if ground_truth != self.game_state:
            #     # self.game_state = ground_truth # overwrite
            #     queue_visualizer_recalibration.put(ground_truth)
            

class EvalClient():
    def __init__(self):
        self.server_ip_address = "localhost"
        self.listening_port = 4010
    ...
    def run(self):
        while True:
            ...
            #         print(f'[RECEIVED GROUND TRUTH FROM EVAL SERVER] {ground_truth}')
            #         #queue_ground_truth.put(ground_truth)
            #         break
...
class Visualizer():
    ...
    def speak(self):
        while True:
            grenade_action_or_game_state = queue_visualizer.get()
            if type(grenade_action_or_game_state) == dict:
                self.publisher.publish("B12cNwWz/Ultra96Send", json.dumps(grenade_action_or_game_state))
                print(f'{Fore.BLUE}[VISUALIZER HAS RECEIVED GAME STATE]{Style.RESET_ALL}')
            else:
                self.publisher.publish("B12cNwWz/Ultra96Send", self.greande_action)
                print(f'{Fore.YELLOW}[VISUALIZER HAS RECEIVED GRENADE ACTION]{Style.RESET_ALL}')
...
ol = Overlay('/usr/local/share/pynq-venv/lib/python3.8/site-packages/pynq/overlays/AI/design_1_wrapper.bit')
dma = ol.axi_dma_0
